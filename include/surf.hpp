...
public:
	...
	create(keys.begin(), keys.end(), kIncludeDense, kSparseDenseRatio, kNone, 0, 0);
	}
    template<typename It>
	SuRF(It i1, It i2) {
	create(i1, i2, kIncludeDense, kSparseDenseRatio, kNone, 0, 0);
	}

    template<typename It>
	SuRF(It i1, It i2, const SuffixType suffix_type,
	 const level_t hash_suffix_len, const level_t real_suffix_len) {
	create(i1, i2, kIncludeDense, kSparseDenseRatio, suffix_type, hash_suffix_len, real_suffix_len);
	}

	~SuRF() {}

	template<typename It>
	void create(It i1, It i2,
		const bool include_dense, const uint32_t sparse_dense_ratio,
		const SuffixType suffix_type,
				const level_t hash_suffix_len, const level_t real_suffix_len) {
	    SuRFBuilder builder_(include_dense, sparse_dense_ratio,
								  suffix_type, hash_suffix_len, real_suffix_len);
		builder_.build(i1, i2);
	...
	SuRF(const std::vector<std::string>& keys, const SuffixType suffix_type,
	 const level_t hash_suffix_len, const level_t real_suffix_len) {
	create(keys.begin(), keys.end(), kIncludeDense, kSparseDenseRatio, suffix_type, hash_suffix_len, real_suffix_len);
	}
	
	SuRF(const std::vector<std::string>& keys,
	 const bool include_dense, const uint32_t sparse_dense_ratio,
	 const SuffixType suffix_type, const level_t hash_suffix_len, const level_t real_suffix_len) {
	create(keys.begin(), keys.end(), include_dense, sparse_dense_ratio, suffix_type, hash_suffix_len, real_suffix_len);
	}
	// This function searches in a conservative way: if inclusive is true
	// and the stored key prefix matches key, iter stays at this key prefix.
